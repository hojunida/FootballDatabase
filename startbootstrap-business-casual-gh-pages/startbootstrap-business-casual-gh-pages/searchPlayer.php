<?php
	...
	if($stmt->prepare("select * from Players where Name like ?") or die(mysqli_error($db))) {
		...
		while($stmt->fetch()) {
			echo "<tr><td>$No</td><td>$Name</td><td>$Ht</td><td>$Wt</td><td>$Class</td><td>$Hometown</td><td>$Pos</td><td>$State</td><td>$Team</td><td>$UID</td><td><input type='checkbox' name='interest' /></td></tr>";
		}
		echo "</table>";
