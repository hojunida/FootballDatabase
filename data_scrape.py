...
cols = ["No", "Name", "Pos", "Ht", "Wt", "Class", "Hometown", "State", "Team", "UID"]            
...
url = 'http://www.espn.com/college-football/team/roster/_/id/2261'
storage = pd.DataFrame(columns = cols) 
    ...
    if "<table" in table and 'class="tablehead"' in table:
        for item in table.split("sortcell"):
            if (not '!DOCTYPE' in item):
                ...
                UID = 5000 + id_index;
                id_index = id_index + 1;
                storage.loc[index] = [no, name, pos, ht, wt, cla, home, state, team, UID]
                index = index + 1
                
writer = pd.ExcelWriter('hamp.xlsx')
storage.to_excel(writer, 'Sheet1')     
writer.save()